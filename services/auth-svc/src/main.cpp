...
std::atomic<uint64_t> g_auth_requests{0};
std::atomic<uint64_t> g_risk_requests{0};
std::atomic<uint64_t> g_auth_errors{0};
...
int main(int argc, char** argv) {
  ...
  int metrics_port = std::stoi(hs::get_env("METRICS_PORT", "9104"));
  ...
  http.Get("/metrics", [](const httplib::Request&, httplib::Response& res){
    std::string body;
    body += "auth_requests_total " + std::to_string(g_auth_requests.load()) + "\n";
    body += "risk_requests_total " + std::to_string(g_risk_requests.load()) + "\n";
    body += "auth_errors_total " + std::to_string(g_auth_errors.load()) + "\n";
