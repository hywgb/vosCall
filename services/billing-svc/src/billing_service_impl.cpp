...
::grpc::Status BillingServiceImpl::Authorize(::grpc::ServerContext*, const AuthorizeRequest* req, AuthorizeResponse* resp){
  try {
    ...
    tx.exec(pqxx::zview("INSERT INTO billing.authorizations(call_id,account_id,token,amount,currency,status) VALUES($1,$2,$3,$4,$5,'open')"),
                   pqxx::params{req->call_id(), acc_id, token, budget, ccy});
    tx.commit();
...
::grpc::Status BillingServiceImpl::Rate(::grpc::ServerContext*, const RateRequest* req, RateResponse* resp){
  try {
    ...
    auto rt = tx.exec(pqxx::zview("SELECT rt.rate_table_id, rt.currency FROM billing.rate_tables rt \n"
                             ...
                             "ORDER BY rt.effective_from DESC LIMIT 1"), pqxx::params{req->account_code()});
    if (rt.empty()) return {::grpc::StatusCode::NOT_FOUND, "rate table not found"};
    auto rate_table_id = rt[0][0].as<long long>();

    auto r = tx.exec(pqxx::zview("SELECT ri.price_per_min, ri.billing_step_sec, ri.min_time_sec, ri.connection_fee, ri.rounding_mode, p.prefix \n"
                             ...
                             "ORDER BY length(p.prefix) DESC LIMIT 1"), pqxx::params{rate_table_id, req->e164_to()});
...
::grpc::Status BillingServiceImpl::Settle(::grpc::ServerContext*, const SettleRequest* req, SettleResponse* resp){
  try {
    ...
    auto r = tx.exec(pqxx::zview("SELECT ri.price_per_min, ri.billing_step_sec, ri.min_time_sec, ri.connection_fee \n"
                             "FROM billing.rate_items ri JOIN routing.prefixes p ON p.prefix_id=ri.prefix_id \n"
                             "WHERE ri.rate_table_id=$1 AND $2 LIKE p.prefix || '%' \n"
                             "ORDER BY length(p.prefix) DESC LIMIT 1"), pqxx::params{rate_table_id, req->e164_to()});
    if (r.empty()) return {::grpc::StatusCode::NOT_FOUND, "rate not found"};
    ...
    // 标记授权为 settled
    tx.exec(pqxx::zview("UPDATE billing.authorizations SET amount=$1, status='settled', updated_at=now() WHERE token=$2"),
                   pqxx::params{amount, req->auth_token()});

    if (prepaid) {
      tx.exec(pqxx::zview("UPDATE core.accounts SET balance = balance - $1, updated_at=now() WHERE account_id=$2"), pqxx::params{amount, account_id});
    } else {
      // 后付：写入发票或应收表（此处简化记入发票金额）
      tx.exec(pqxx::zview("INSERT INTO billing.invoices(account_id, period_start, period_end, currency, amount_due, status) VALUES ($1, date_trunc('month', now()), date_trunc('month', now()) + interval '1 month' - interval '1 day', $2, $3, 'open') ON CONFLICT DO NOTHING"),
                     pqxx::params{account_id, req->currency(), amount});
    }
